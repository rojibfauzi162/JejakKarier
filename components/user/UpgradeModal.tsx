...
const UpgradeModal: React.FC<UpgradeModalProps> = ({ products, currentPlan, onClose, onSelectPlan, userEmail, userName }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-0 sm:p-4 lg:p-2 sm:p-6 animate-in fade-in duration-300">
      <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onClick={onClose}></div>
      
      <div className="relative bg-white w-full max-w-5xl rounded-[2rem] md:rounded-[3rem] lg:rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in duration-500 max-h-[95vh] flex flex-col">
        {/* Header Section */}
        <div className="bg-indigo-600 p-4 md:p-8 lg:p-14 text-white relative overflow-hidden shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 md:p-6 lg:p-14 no-scrollbar bg-white">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 lg:gap-4 md:gap-8 items-stretch pb-4">
              {premiumProducts.map(p => {
                const isYearly = p.durationDays >= 365;
                const monthlyPrice = p.durationDays >= 30 ? Math.round(p.price / (p.durationDays / 30)) : null;

                return (
                  <div key={p.id} className={`p-4 md:p-8 rounded-[3.5rem] border-2 flex flex-col h-full transition-all duration-700 relative group hover:shadow-2xl ${isYearly ? 'bg-slate-900 text-white border-slate-900 shadow-xl' : 'bg-white text-slate-900 border-slate-100 hover:border-indigo-600'}`}>

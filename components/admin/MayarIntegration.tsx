...
const MayarIntegration: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl space-y-4 md:space-y-8 animate-in fade-in duration-500 pb-20">
      {toast && (
        <div className={`fixed top-10 right-10 z-[3000] px-5 py-3 md:px-8 md:py-4 rounded-2xl shadow-2xl border text-white font-black text-[10px] uppercase tracking-widest animate-in slide-in-from-right-4 ${toast.t === 's' ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-rose-600 border-rose-500 text-white'}`}>
           {toast.m}
        </div>
      )}
      ...
      <div className="bg-white p-4 md:p-10 lg:p-14 rounded-[3.5rem] shadow-sm border border-slate-100">
         ...
         <form onSubmit={handleSave} className="space-y-3 md:space-y-6 md:space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Environment</label>
                 <select 
                  className="w-full px-4 py-3 md:px-6 md:py-4 rounded-2xl bg-white border border-slate-200 outline-none font-bold text-xs"
              ...
              <div className="space-y-2">
                 ...
                 <input 
                  type="password" 
                  className="w-full px-4 py-3 md:px-6 md:py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-xs focus:border-blue-400 transition-all" 
            ...
            <div className="space-y-2">
               ...
               <div className="flex items-center gap-2">
                  <input 
                    type="text" 
                    className="flex-1 px-4 py-3 md:px-6 md:py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-xs focus:border-blue-400 transition-all" 
            ...
            <div className="space-y-3 md:space-y-6">
               ...
               <div className="bg-slate-50/50 p-4 md:p-8 rounded-[2.5rem] border border-slate-100 space-y-4">
      ...
      <div className="bg-white p-4 md:p-10 rounded-[3rem] shadow-sm border border-slate-100 flex items-start gap-3 md:gap-6">

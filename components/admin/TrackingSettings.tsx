...
const TrackingSettings: React.FC<TrackingSettingsProps> = ({ onToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // Remove eventMapping to use default settings
      const { eventMapping, ...configToSave } = config;
      await saveTrackingConfig(configToSave);
      // Re-initialize tracking service immediately
      trackingService.init(configToSave);

...
const DailyLogs: React.FC<DailyLogsProps> = ({ logs, categories, currentCompany, onAdd, onUpdate, onDelete, onAddCategory, onUpdateCategory, onDeleteCategory, affirmation, targetDate, appData, onUpgrade }) => {
  ...
  const totalPages = Math.ceil(groupedLogsByDate.length / itemsPerPage);
  const paginatedDays = useMemo(() => {
    return groupedLogsByDate.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  }, [groupedLogsByDate, currentPage, itemsPerPage]);
      ...
      <div className="space-y-8">
        {paginatedDays.length > 0 ? (
          paginatedDays.map((dayGroup) => (
            <div key={dayGroup[0]} className="bg-white rounded-[3rem] shadow-sm border border-slate-200 overflow-hidden min-h-[200px] flex flex-col animate-in fade-in duration-700">
                ...
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h3 className="text-lg font-black text-slate-900 tracking-tight leading-none uppercase">
                          {new Date(dayGroup[0]).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                      </h3>
                   </div>
                </div>
                <button 
                  type="button"
                  onClick={() => handleOpenModal(dayGroup[1][0], true)} 
          ...
          <div className="bg-white rounded-[3rem] shadow-sm border border-slate-200 flex flex-col items-center justify-center py-32 space-y-3 md:space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-8 bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tampilkan:</span>
            <select 
              className="px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold text-[10px] cursor-pointer"
              value={itemsPerPage}
            ...
            <span className="text-[10px] font-bold text-slate-400 tracking-tight">Total {groupedLogsByDate.length} Hari</span>
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button 
                ...
                className="px-3 h-9 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-200 text-slate-600 disabled:opacity-30 hover:bg-slate-100 transition-all font-black text-xs"
              >
                &lt;&lt;
              </button>
                ...
                {Array.from({ length: totalPages }).map((_, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-9 h-9 flex items-center justify-center rounded-xl font-black text-xs transition-all ${currentPage === pageNum ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100'}`}
                    >
                      {pageNum}
                    </button>
                  );

...
app.post("/createInquiry", async (req, res) => {
  try {
    ...
    // Pastikan nominal adalah integer murni
    ...
    console.log("[DUITKU] Inquiry Payload:", JSON.stringify(payload));
    ...
    try {
      ...
      return res.status(502).json({
        statusCode: "502",
        statusMessage: "Duitku memberikan respon tidak valid.",
      });
    }
    ...
    return res.status(500).json({
      statusCode: "500", 
      statusMessage: "Kesalahan server: " + error.message
    });
  }
});
    ...
    if (resultCode === "00") {
      ...
      if (userSnap.exists) {
        ...
        if (txIndex > -1 && transactions[txIndex].status !== "Paid") {
          ...
          console.log(`[DUITKU CALLBACK] User ${additionalParam} activated.`);
        }
      }
    }
